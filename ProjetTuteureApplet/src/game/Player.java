...
public class Player extends Combattant implements Mover {
	private static final long serialVersionUID = 1786406332280808196L;
	...
	public String listeInvitationHTML(){
		...
		for(int i=listeInvitation.size()-1; i>=0; i--){
			...
			res+="<li>"+g.getNom()+", par "+g.getLeader().getNom()+". <input name='Rejoindre' type='button' value='Oui' onclick='rejoindre("+i+")'/> <input name='Refuser' type='button' value='Non' onclick='nepasrejoindre("+i+")'/> </li>";
		}
		return res+"</ul>";
	}
